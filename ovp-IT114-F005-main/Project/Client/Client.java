...
public enum Client {
    ...
    private boolean processClientCommand(String text) throws IOException {
        ...
        {  
            ...
            if (parts.length == 2) 
            {
                ...
                payload.setMessage(parts[1]);                       // ovp 12/22
                send(payload);
                LoggerUtil.INSTANCE.info("Client muted: ");

            } else {
                chatArea.add(chatArea, "Use /mute (username).");
            }
        } if (text.startsWith("/unmute")) 
        {
            ...
            if (parts.length == 2) 
            {
                ...
                payload.setMessage(parts[1]); 
                send(payload);
                LoggerUtil.INSTANCE.info("Client unmuted: ");
            } else {
                chatArea.add(chatArea,"Use /unmute (username)");
            }
        } if (text.startsWith("@")) {
            text = text.trim(); 
            String[] parts = text.split(" ", 2); 
            if (parts.length == 2) {
                String target = parts[0].substring(1).toLowerCase();
    ...
    private void rollCommand(String val) throws IOException 
    {
        if (val.contains("d")) 
        {
        ...
        else 
        {
            ...
            RollPayload rollPayload = new RollPayload(numSides, 0);  
            rollPayload.setPayloadType(PayloadType.ROLL);
            send(rollPayload);
        }
    }
    
    private void flipCommand() throws IOException
    {
    ...
    private void processDisconnect(long clientId, String clientName) {
        ...
        if (clientId == myData.getClientId()) {
            closeServerConnection();                        // TODO see if this doesnt work
        }
    }
    ...
    private void displayPrivateMessage(String senderId, String message) {
        String formattedMessage = "(PM) " + senderId + ": " + message;
