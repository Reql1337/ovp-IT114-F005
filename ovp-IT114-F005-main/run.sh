...
if [ "$input" = "server" ]; then
    /c/Program\ Files/Java/jdk-20/bin/java $1.Server.Server $port
elif [ "$input" = "client" ]; then
    /c/Program\ Files/Java/jdk-20/bin/java $1.Client.Client
    # In Milestone3 changes Client to ClientUI
elif [ "$input" = "ui" ]; then
	/c/Program\ Files/Java/jdk-20/bin/java $1.Client.ClientUI
